...
- `createQueueActions(story, state, li)` has an early-return branch for Finalized_Stories that appends the Delete_Control and then the Re_Vote_Control when `state.youAreModerator` is true, then returns. The final estimate pill is built by `createFinalChip(story)` and appended by `createQueueTitleRow` immediately after the story number pill, so it is card metadata and not part of the action area.
...
6. WHEN the Queue_Renderer renders a Story_Card for a Finalized_Story, THE Queue_Renderer SHALL omit the Vote control and the edit control for every viewing role, so that the finalized card action set is empty for a Participant and is the Delete_Control followed by the Re_Vote_Control for the Facilitator, with the final estimate pill rendered in the title row in both cases.
